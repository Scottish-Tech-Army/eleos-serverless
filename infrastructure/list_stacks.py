...
def print_output():
    ...
    for i in range(len(stacks["Stacks"])):
        print(
            f" {i}: "
            + stacks["Stacks"][i]["StackName"]
            + label[0]
            + stacks["Stacks"][i]["StackStatus"]
            + label[1]
            + stacks["Stacks"][i]["CreationTime"].strftime("%d/%m/%Y")
        )
    ...
    while True:
        ...
        try:
            ...
            try:
                print(
                    label[4]
                    + stacks["Stacks"][item]["LastUpdatedTime"].strftime("%d/%m/%Y")
                    + label[5]
                    + stacks["Stacks"][item]["LastUpdatedTime"].strftime("%H:%M:%S")
                )
            except:
                pass
            try:
                ...
                for i in range(len(stacks["Stacks"][item]["Outputs"])):
                    print(
                        " "
                        + stacks["Stacks"][item]["Outputs"][i]["OutputKey"]
                        + ":     "
